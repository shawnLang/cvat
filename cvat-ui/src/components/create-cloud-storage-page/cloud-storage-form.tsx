...
export default function CreateCloudStorageForm(props: Props): JSX.Element {
    ...
    useEffect(() => {
        if (cloudStorageId && cloudStorage.credentialsType !== CredentialsType.ANONYMOUS_ACCESS) {
            notification.info({
                message: `出于安全原因，您的凭据将被隐藏并由假值表示.
                    这些值在更新云存储时不会被考虑在内.
                    如果要替换原始凭据，只需输入新的凭据即可.`,
            ...
            return (
                <>
                    <Form.Item
                        label='Access key ID'
                        name='key'
                        rules={[{ required: true, message: '请指定您的access key ID' }]}
                        {...internalCommonProps}
                    >
                        <Input.Password
                            maxLength={128}
                    ...
                    <Form.Item
                        ...
                        name='secret_key'
                        rules={[{ required: true, message: '请指定您的secret access key' }]}
                        {...internalCommonProps}
                    >
                        <Input.Password
                            maxLength={64}
            ...
            return (
                <>
                    <Form.Item
                        label='Account name'
                        name='account_name'
                        rules={[{ required: true, message: '请填写您的account name' }]}
                        {...internalCommonProps}
                    >
                        <Input.Password
                            minLength={3}
                    ...
                    <Form.Item
                        label='SAS token'
                        name='SAS_token'
                        rules={[{ required: true, message: '请填写您的SAS token' }]}
                        {...internalCommonProps}
                    >
                        <Input.Password
        ...
        if (providerType === ProviderType.AZURE_CONTAINER && credentialsType === CredentialsType.CONNECTION_STRING) {
            return (
                <Form.Item
                    ...
                    rules={[{ required: true, message: '请填写您的connection string' }]}
                    {...internalCommonProps}
                >
                    <Input.Password
                        maxLength={1024}
        ...
        return (
            <>
                <Form.Item
                    label='Bucket name'
                    name='resource'
                    rules={[{ required: true, message: '请填写您的bucket name' }]}
                    {...internalCommonProps}
                >
                ...
                <Form.Item
                    ...
                    rules={[{ required: true, message: '请填写您的credentials type' }]}
                    {...internalCommonProps}
                >
                ...
                <Form.Item
                    label='Endpoint URL'
                    help='您可以在使用 AWS S3 云存储兼容 API 时为存储指定终端节点'
                    name='endpoint_url'
                    {...internalCommonProps}
                >
                    <Input />
        ...
        return (
            <>
                <Form.Item
                    ...
                    name='resource'
                    rules={[{ required: true, message: '请填写您的container name' }]}
                    {...internalCommonProps}
                >
                    <Input disabled={!!cloudStorage} maxLength={63} />
                </Form.Item>
    ...
    return (
        ...
        >
            <Form.Item
                ...
                label='显示名称'
                name='display_name'
                rules={[{ required: true, message: '请填写您的显示名称' }]}
            >
                <Input maxLength={63} />
            </Form.Item>
            <Form.Item {...commonProps} label='描述' name='description'>
                <TextArea autoSize={{ minRows: 1, maxRows: 5 }} placeholder='任何有用的描述' />
            </Form.Item>
            <Form.Item
                {...commonProps}
                label='供应商'
                name='provider_type'
                rules={[{ required: true, message: '请指定云存储提供商' }]}
            >
                <Select
            ...
            <Form.Item
                label={(
                    <CVATTooltip title='Prefix 用于筛选存储桶内容'>
                        Prefix
